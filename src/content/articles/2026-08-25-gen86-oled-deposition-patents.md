...
sources:
  ...
  - type: patent
    title: "유기 발광 표시 장치 및 그 제조 방법 (KR102700938B1, 삼성디스플레이, 2022-12-23 출원 / 2024-09-02 등록). 파인메탈마스크 없이 발광층을 형성하는 포토리소그래피 방식"
    url: "https://patents.google.com/patent/KR102700938B1/en"
featured: false
paywallAfter: 0
---
...
한 가지 덧붙일 것이 있다. 이 기사가 다룬 세 층 가운데 마스크 층 자체를 지우려는 특허가 이미 등록돼 있다. 삼성디스플레이가 2022년 12월에 출원해 2024년 9월에 등록받은 KR102700938B1은 화소정의막 위에 불소를 함유한 자기조립 단분자막을 깔고, 그 위에 리프트오프층과 포토레지스트를 올려 포토마스크로 노광하는 방식으로 발광층을 만든다. 명세서가 밝힌 효과는 이렇다.

<div class="quote-box"><p>미세 금속 마스크(fine metal mask: FMM)를 사용하지 않고 발광층을 형성하기 때문에 고해상도 표시 패널을 형성할 수 있다.</p><cite>KR102700938B1 명세서</cite></div>

다만 이 명세서가 FMM을 없애는 이유로 든 것은 해상도이지 대면적의 처짐이 아니다. 두 문제가 같은 수단으로 함께 풀린다는 서술은 이 명세서에서 확인되지 않는다. 그 점은 구분해서 읽어야 한다.

그래도 방향은 읽힌다. 마스크를 크게 만드는 일이 어려워질수록 마스크를 쓰지 않는 쪽의 값이 올라간다. 다음 편에서는 그쪽 특허를 다룬다. 포토리소그래피와 잉크젯 인쇄, 마스크를 지우는 두 갈래가 같은 자리를 어떻게 다르게 겨냥하는지 보기 위해서다.
