...
function stripComments(src) {
  return src
    // 🔴 줄바꿈을 먼저 통일한다. CRLF면 split('\n') 뒤 각 줄 끝에 \r가 남는데,
    // 정규식의 `.`은 \r를 매치하지 않아 아래 `/\s\/\/.*$/` 가 통째로 실패한다.
    // 그러면 줄 끝 주석이 안 지워져 그 한글이 「캔버스 안 한글」로 오탐된다.
    // 전체줄 주석은 `/^\s*\/\//` 라 영향이 없어서, 줄 끝 주석만 골라 터졌다.
    // (2026-09-13 테커가 제5호 AG 편 작업 중 두 번 걸려 보고)
    .replace(/\r\n?/g, '\n')
    .replace(/\/\*[\s\S]*?\*\//g, ' ')
    .split('\n')
